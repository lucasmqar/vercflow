    ...
    particles.forEach(particle => {
        ...
        if (shouldVaporize) {
            ...
            if (particle.shouldFadeQuickly) particle.opacity = Math.max(0, particle.opacity - deltaTime);
                ...
                particle.velocityX = (particle.velocityX + (Math.random() - 0.5) * spread + dx * 0.002) * damping;
                particle.velocityY = (particle.velocityY + (Math.random() - 0.5) * spread + dy * 0.002) * damping;
...
function transformValue(input: number, inputRange: number[], outputRange: number[], clamp = false): number {
    const [inMin, inMax] = inputRange, [outMin, outMax] = outputRange;
    const progress = (input - inMin) / (inMax - inMin);
    let res = outMin + progress * (outMax - outMin);
    if (clamp) res = outMax > outMin ? Math.min(Math.max(res, outMin), outMax) : Math.min(Math.max(res, outMax), outMin);
    return res;
}
