...
export function ActivitiesDashboard() {
    ...
    return (
        <div className="h-full flex flex-col bg-background/50 font-sans overflow-hidden">
            {/* Standardized Header - Modern Technical */}
            <div className="px-6 py-4 border-b border-border/40 bg-background/5 backdrop-blur-md flex items-center justify-between">
                <div>
                    <h1 className="text-base font-bold tracking-tight text-foreground flex items-center gap-2">
                        <ActivityIcon size={18} className="text-primary/70" />
                        Fluxo de Atividades
                    </h1>
                    <p className="text-[11px] text-muted-foreground font-medium uppercase tracking-wider opacity-60">Visão Técnica e Operacional</p>
                </div>
                    ...
                    <div className="flex gap-1 bg-muted/20 rounded-lg p-1 border border-border/40">
                        {[
                            ...
                            <button
                                ...
                                className={cn(
                                    "h-8 px-3 rounded-md flex items-center gap-2 text-xs font-semibold transition-all transition-all duration-200",
                                    viewMode === mode.id
            ...
            <div className="flex-1 overflow-x-auto p-6 bg-muted/5 h-full">
                {/* Kanban View */}
                {viewMode === 'kanban' && (
                                        ...
                                        >
                                            <td className="p-4">
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <span className="text-[10px] font-mono text-muted-foreground/60 uppercase">Project: {activity.project?.nome || 'Geral'}</span>
                                                </div>
                                            </td>
                                            <td className="p-4">
                                                {activity.record ? (
                ...
                {/* Grid View, Calendar, Timeline - Porting new styles */}
                {viewMode === 'grid' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 h-full overflow-auto pb-4 scrollbar-thin">
                        {activities.map((activity) => (
                            <Card
                                key={activity.id}
