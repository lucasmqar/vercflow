...
interface RegistrosState {
    ...
    updateRecordItem: (id: string, checked: boolean) => Promise<void>;
}
...
export const useRegistros = create<RegistrosState>((set, get) => ({
    ...
    updateActivityStatus: async (id, status) => {
        try {
            ...
            if (!response.ok) throw new Error('Falha ao atualizar status da atividade');
    ...
    updateRecordItem: async (id, checked) => {
        const registros = get().registros;
        // Optimistic update
        set({
            registros: registros.map(r => ({
                ...r,
                items: r.items.map(i => i.id === id ? { ...i, checked } : i)
            }))
        });
